...
def conversion_rate_plot(converted_series):
    ax = converted_series.plot(grid=True, title ="Conversion Rate")
    ax.set_xlabel("Segment")
    ax.set_ylabel("Conversion rate %")
    plt.show()

def conversion_rate_bar_plot(converted_series):
    ax = converted_series.plot(kind='bar', figsize=(10, 7), color ="skyblue", grid=True, title='Conversion Rates by Age Groups')
    ax.set_ylabel('conversion rate (%)')
    ax.set_xlabel('age')
    plt.show()
